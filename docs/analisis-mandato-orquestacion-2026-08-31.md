...
| **PR-5 adversario con fs** | Independencia (H1c) | Atacante con escritura en el medio reescribe store+índices+CURRENT coherentemente → `verify` NO detecta (predicción negativa honesta). Con export sellado + digest anclado fuera del alcance del atacante → sí detecta | Red-team G-3: script de ataque + verificación contra digest externo (p. ej. digest commiteado en otro repo/medio) |
